"""Run pending Privacy Leakage Cheap Test A configs through the shared recorder."""
...
CONFIGS = ROOT / "experiments" / "configs" / "privacy_leakage_v1"
...
def main() -> int:
    ...
    for path in sorted(CONFIGS.glob("*.json")):
        ...
        if config["experiment_id"] in existing:
            print("skip", config["experiment_id"])
            continue
