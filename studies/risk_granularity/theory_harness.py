...
def evaluate_configuration(params,seed,thresholds,epsilons):
 ...
 for region in regions:
  tree=GranulationTree(random_state=17+seed,split_method=params.get('generation_method','kmeans')).fit(Xtr[rtr==region],ytr[rtr==region]);rows=[]
 ...
 return {'accuracy':float(accuracy_score(true,pred)),'macro_f1':float(f1_score(true,pred,average='macro')),'nll':float(log_loss(true,prob,labels=[0,1])),'brier':float(brier_score_loss(true,prob[:,1])),'granules':primary['cost'],'compression_ratio':primary['cost']/max_cost if max_cost else 1.,'memory_bytes_estimate':primary['cost']*bytes_per_granule,'runtime_seconds_inner':time.perf_counter()-start,'global_frontier':global_with_regret,'oracle_frontier':oracle,'risk_budget_points':budgeted,'max_granules':max_cost,'global_positive_regret_fraction':float(np.mean([(x['frontier_regret'] or 0)>1e-12 for x in global_with_regret])),'global_mean_regret':float(np.mean([x['frontier_regret'] or 0 for x in global_with_regret])),'generation_method':params.get('generation_method','kmeans'),'meta':meta}
